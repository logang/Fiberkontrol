...
#----Plot all data on same plot---#
        ...
        plt.figlegend(self.plotFullRange, self.filename, 'upper right')
...
#        self.ynormFlat = array(self.yflat)/self.yinit
...
#        if self.numInThemesSoFar[self.theme] == 1:
 #           setp(figHist, frameon=False)
  #          setp(figHist.get_axes(), visible=False)

   #     axNew = figHist.add_subplot(self.totalOfThemes[self.theme],1,self.numInThemesSoFar[self.theme])

#        self.plot_hist(axNew, self.mouseLabel, self.ynorm)
        self.plot_hist(figHist, self.mouseLabel, self.ynorm)
...
#--Plot flattened histograms
    ...
    def plot_hist(self, fig, mouseLabel, yData):
        ...
        self.plotHist = ax.hist(yData, bins=1000, range = (0.85, 1.1), histtype='bar')

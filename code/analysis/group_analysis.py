...
def compare_start_and_end_of_epoch(all_data, options, exp1='homecagesocial', exp2='homecagenovel', time_window=[0,0.25], metric='area', test='ttest', plot_perievent=False, compare_before_after_end=False):
...
def get_bout_averages(pair_scores):
    ...
    returns: 1) bout_dict = a dict (keys: exp_type, entries: a dict (keys: bout number, entries: array with score from each mice for that bout number)
